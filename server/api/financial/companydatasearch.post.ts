...
export default defineEventHandler(async (event: H3Event) => {
    try {
        ...
        if (process.dev) {
            console.log(`[Financial API Proxy] Making request to ${apiBaseUrl}/companydatasearch with query:`, query.query)
        }

        // Make the request to the actual API
        const response = await $fetch(`${apiBaseUrl}/companydatasearch`, {
            method: 'POST',
            query: query,
            headers: {
